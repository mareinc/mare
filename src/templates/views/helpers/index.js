...
module.exports = function() {
	...
	_helpers.date = function date(context, options) {
	...
	_helpers.categoryList = function categoryList(categories, options) {
	...
	_helpers.isAdminEditorCSS = function isAdminEditorCSS(user, options) {
		var output = '';
	...
	_helpers.isAdminEditorJS = function isAdminEditorJS(user, options) {
		var output = '';
		if (typeof(user) !== 'undefined' && user.isAdmin) {
	...
	* */
	_helpers.ifHasPagination = function ifHasPagination(postContext, options){
	...
	_helpers.paginationNavigation = function paginationNavigation(pages, currentPage, totalPages, options){
		var html = '';
	...
	_helpers.flashMessages = function flashMessages(messages) {
		var output = '';
		for (var i = 0; i < messages.length; i++) {

			// open a containing div
			output += '<div class="alert__content"><div class="alert__content-message">';
			...
			// add a close button and close the containing div
			output += '</div><span class="flash-message__close-button">x</span></div>';
		}
		return new hbs.SafeString(output);
	};
