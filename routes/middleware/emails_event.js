...
exports.sendEventRegistrationEmailToStaff = ( registrationInfo, registrationStaffContact ) => {
	...
	return new Promise( ( resolve, reject ) => {
		...
		new keystone.Email({
			...
			if( response && [ 'rejected', 'invalid', undefined ].includes( response.status ) ) {
				...
				return reject( `new event registration email email to staff failed to send: ${ message } - ${ err }` );
			}
