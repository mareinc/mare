...
/**
 * @JARED: I dont like having this function as middleware as it really isn't one but 
 * 	I am trying to keep conformity with the login and logout sections
 * @param {*} req 
 * @param {*} res 
 * @param {*} next 
 */
	...
	UserMiddleware.getUserByEmail(req.body.email)
				  .then( user => {
					...
					if(!user){
						...
						req.flash('error', {title:'Something went wrong',
										 detail: 'We could not get the user account.'});
					}
					else{
						console.log(user);

						//reset the user password out here. resetPassword function doesnt exist yet
						//there doesnt seem to be any place to change the user password so the method of having a one time key
						//may be the appropriate one out here
